		...
		for ch in 0 ..< 2 {
			...
			for r in 0 ..< 3 {
				for c in 0 ..< 3 {
					...
					rl.DrawRectangle(
						ch_x + i32(c) * cell_size,
						offset_y + i32(r) * cell_size,
						cell_size - 1,
						cell_size - 1,
						color,
					)
...
draw_lx_conv_weights :: proc(
	weights: brain.LX_CONV_WEIGHTS,
	label: cstring,
	start_x: i32,
	start_y: i32,
) {
	...
	for f in 0 ..< brain.filter_count {
		...
		for kr in 0 ..< 3 {
			for kc in 0 ..< 3 {
				...
				rl.DrawRectangle(
					offset_x + i32(kc) * cell_size,
					offset_y + i32(kr) * cell_size,
					cell_size - 1,
					cell_size - 1,
					color,
				)
...
draw_dense_weights :: proc(weights: brain.DENSE_WEIGHTS, start_x: i32, start_y: i32) {
	...
	for i in 0 ..< len(weights[0]) {
		color := weight_to_color(weights[0][i], 0.05)
		rl.DrawRectangle(
			start_x + i32(i % cols) * cell_size,
			start_y + i32(i / cols) * cell_size,
			cell_size - 1,
			cell_size - 1,
			color,
		)
